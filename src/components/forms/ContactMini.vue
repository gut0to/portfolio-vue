...
const name = ref('')
const email = ref('')
const service = ref('Site Vue')
const budget = ref('2-5k')
const deadline = ref('2-4 semanas')
const message = ref('')
...
<template>
  <form class="space-y-4">
    ...
    <textarea v-model="message" rows="5" placeholder="Fale brevemente sobre o projeto..." class="px-4 py-3 rounded-xl bg-neutral-900 border border-neutral-800 w-full"></textarea>

    <div class="flex flex-wrap gap-3">
      <button type="button" @click="send('mail')" class="px-5 py-3 rounded-xl bg-fuchsia-600 hover:bg-fuchsia-500 text-white">Enviar por e-mail</button>
      <button type="button" @click="send('wa')" class="px-5 py-3 rounded-xl border border-neutral-700 hover:bg-neutral-900">Falar no WhatsApp</button>
    </div>
  </form>
</template>
